...
def load_args(default_config=None):
    ...
    # -- other vars
    parser.add_argument('--interval', default=500, type=int, help='display interval')
    ...
    # use PHM
    parser.add_argument('--h', action='store_true', default=False, help='use parameterized hypercomplex layers')
    parser.add_argument('--f', action='store_true', default=False, help='use parameterized hypercomplex layers in the frontend')
    parser.add_argument('--n', type=int, default=4, help='set dimension N (grouping factor)')

    parser.add_argument('--ablate_b', action='store_true', default=False, help='skip PHM layers in the backend')
    parser.add_argument('--ablate_t', action='store_true', default=False, help='skip PHM layers in the temporal net')

    parser.add_argument('--expn', type=str)
...
def evaluate(model, dset_loader, criterion):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(tqdm(dset_loader)):
            ...
            #         input = input.unsqueeze(1).expand(-1, args.n, -1)
            #     else:
            #         input = input.unsqueeze(1).expand(-1, args.n, -1, -1, -1)
            #     logits = model(input.cuda(), lengths=lengths, boundaries=boundaries)
            # else:
            ...
            if args.f:
                if args.modality == 'audio':
                    pad = (0,0, 0,args.n - 1, 0,0)
                else:
                    pad = (0,0, 0,0, 0,0, 0,args.n - 1, 0,0)
                input = F.pad(input.unsqueeze(1), pad)
                logits = model(input.cuda(), lengths=lengths, boundaries=boundaries)
            else:
                logits = model(input.unsqueeze(1).cuda(), lengths=lengths, boundaries=boundaries)
...
def get_model_from_json():
    ...
    model = Lipreading( modality=args.modality,
                        ...
                        phm=args.h, phmf=args.f, n=args.n, ablate_b=args.ablate_b, ablate_t=args.ablate_t
                        ).cuda()
