...
from lipreading.models.layers import PHMConv3d
...
class MultiscaleMultibranchTCN(nn.Module):
    def __init__(self, input_size, num_channels, num_classes, tcn_options, dropout, relu_type, dwpw=False, phm=False, n=4):
        super(MultiscaleMultibranchTCN, self).__init__()

        self.kernel_sizes = tcn_options['kernel_size']
        self.num_kernels = len( self.kernel_sizes )

        self.mb_ms_tcn = MultibranchTemporalConvNet(input_size, num_channels, tcn_options, dropout=dropout, relu_type=relu_type, dwpw=dwpw, phm=phm, n=n)
...
class TCN(nn.Module):
    ...
    def __init__(self, input_size, num_channels, num_classes, tcn_options, dropout, relu_type, dwpw=False, phm=False, n=4):
        ...
        self.tcn_trunk = TemporalConvNet(input_size, num_channels, dropout=dropout, tcn_options=tcn_options, relu_type=relu_type, dwpw=dwpw, phm=phm, n=n)
        self.tcn_output = nn.Linear(num_channels[-1], num_classes)
...
class DenseTCN(nn.Module):
    def __init__( self, block_config, growth_rate_set, input_size, reduced_size, num_classes,
        ...
        self.tcn_trunk = DenseTemporalConvNet( block_config, growth_rate_set, input_size, reduced_size, kernel_size_set, dilation_size_set, dropout=dropout, relu_type=relu_type, squeeze_excitation=squeeze_excitation, phm=phm, n=n, use_boundary=use_boundary,)
...
class Lipreading(nn.Module):
    def __init__( self, modality='video', hidden_dim=256, backbone_type='resnet', num_classes=500,
                  ...
                  use_boundary=False, extract_feats=False, phm=False, phmf=False, n=4, ablate_b=False, ablate_t=False):
        super(Lipreading, self).__init__()
        self.extract_feats = extract_feats
        self.backbone_type = backbone_type
        self.modality = modality
        self.use_boundary = use_boundary
        self.phm = phm and not ablate_t
        ...
        if self.modality == 'audio':
            ...
            if self.backbone_type == 'resnet':
                ...
                self.backend_out = 512
                if not phm or ablate_b:
                    self.trunk = ResNet(BasicBlock, [2, 2, 2, 2], relu_type=relu_type)
                else:
                    self.trunk = ResNet(BasicBlock, [2, 2, 2, 2], relu_type=relu_type, phm=True, n=n)
            ...
            if not phmf:
                ...
                self.frontend3D = nn.Sequential(
                            PHMConv3d(n, n, self.frontend_nout, kernel_size=(5, 7, 7), stride=(1, 2, 2), padding=(2, 3, 3), bias=False),
                            nn.BatchNorm3d(self.frontend_nout),
                            frontend_relu,
                            nn.MaxPool3d( kernel_size=(1, 3, 3), stride=(1, 2, 2), padding=(0, 1, 1)))
        else:
