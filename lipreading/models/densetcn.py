...
class TemporalConvLayer(nn.Module):
    def __init__(self, n_inputs, n_outputs, kernel_size, stride, dilation, padding, relu_type, phm=False, n=4):
        ...
        if not phm:
            ...
            self.net = nn.Sequential(
                    PHMConv1d(n, n_inputs, n_outputs, kernel_size,
                            stride=stride, padding=padding, dilation=dilation),
                    nn.BatchNorm1d(n_outputs),
                    Chomp1d(padding, True),
                    nn.PReLU(num_parameters=n_outputs) if relu_type == 'prelu' else Swish() if relu_type == 'swish' else nn.ReLU(),)
...
class _ConvBatchChompRelu(nn.Module):
    def __init__(self, n_inputs, n_outputs, kernel_size_set, stride, dilation, dropout, relu_type, se_module=False, phm=False, n=4):
        ...
        for k_idx,k in enumerate( kernel_size_set ):
            if se_module:
                if not phm:
                    ...
                    from lipreading.models.se_module import PHMSELayer
                    setattr( self, 'cbcr0_se_{}'.format(k_idx), PHMSELayer(n, n_inputs, reduction=16))
            cbcr = TemporalConvLayer( n_inputs, self.n_outputs_branch, k, stride, dilation, (k-1)*dilation, relu_type, phm=phm, n=n)
            setattr( self,'cbcr0_{}'.format(k_idx), cbcr )
        self.dropout0 = nn.Dropout(dropout)
        for k_idx,k in enumerate( kernel_size_set ):
            cbcr = TemporalConvLayer( n_outputs, self.n_outputs_branch, k, stride, dilation, (k-1)*dilation, relu_type, phm=phm, n=n)
...
class _Transition(nn.Sequential):
    def __init__(self, num_input_features, num_output_features, relu_type, phm=False, n=4, use_boundary = False):
        super(_Transition, self).__init__()
        if not phm:
            self.add_module('conv', nn.Conv1d(num_input_features, num_output_features,
                                            kernel_size=1, stride=1, bias=False))
        else:
            self.add_module('conv', PHMConv1d(n, num_input_features, num_output_features,
                                            kernel_size=1, stride=1, bias=False, boundary=use_boundary))
...
class DenseTemporalConvNet(nn.Module):
    def __init__(self, block_config, growth_rate_set, input_size, reduced_size,
        ...
        trans = _Transition(num_input_features=input_size,
                            ...
                            relu_type='prelu', phm=phm, n=n, use_boundary=use_boundary)
