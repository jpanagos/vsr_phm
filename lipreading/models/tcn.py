...
class ConvBatchChompRelu(nn.Module):
    def __init__(self, n_inputs, n_outputs, kernel_size, stride, dilation, padding, relu_type, dwpw=False, phm=False, n=4):
        ...
        if dwpw:
            if not phm:
                ...
                self.conv = nn.Sequential(
                    # -- dw
                    PHMConv1d(n, n_inputs, n_inputs, kernel_size, stride=stride,
                            padding=padding, dilation=dilation, groups=n_inputs, bias=False),
                    nn.BatchNorm1d(n_inputs),
                    Chomp1d(padding, True),
                    nn.PReLU(num_parameters=n_inputs) if relu_type == 'prelu' else nn.ReLU(inplace=True),
                    # -- pw
                    PHMConv1d(n, n_inputs, n_outputs, 1, 1, 0, bias=False),
                    nn.BatchNorm1d(n_outputs),
                    nn.PReLU(num_parameters=n_outputs) if relu_type == 'prelu' else nn.ReLU(inplace=True)
                )
        else:
            if not phm:
                ...
                self.conv = PHMConv1d(n, n_inputs, n_outputs, kernel_size,
                                                stride=stride, padding=padding, dilation=dilation)
...
class MultibranchTemporalBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, kernel_sizes, stride, dilation, padding, dropout=0.2,
                 relu_type = 'relu', dwpw=False, phm=False, n=4):
        ...
        for k_idx,k in enumerate( self.kernel_sizes ):
            cbcr = ConvBatchChompRelu( n_inputs, self.n_outputs_branch, k, stride, dilation, padding[k_idx], relu_type, dwpw=dwpw, phm=phm, n=n)
            setattr( self,'cbcr0_{}'.format(k_idx), cbcr )
        self.dropout0 = nn.Dropout(dropout)

        for k_idx,k in enumerate( self.kernel_sizes ):
            cbcr = ConvBatchChompRelu( n_outputs, self.n_outputs_branch, k, stride, dilation, padding[k_idx], relu_type, dwpw=dwpw, phm=phm, n=n)
        ...
        if not phm:
            ...
            self.downsample = PHMConv1d(n, n_inputs, n_outputs, 1) if (n_inputs//self.num_kernels) != n_outputs else None
...
class MultibranchTemporalConvNet(nn.Module):
    def __init__(self, num_inputs, num_channels, tcn_options, dropout=0.2, relu_type='relu', dwpw=False, phm=False, n=4):
        ...
        for i in range(num_levels):
            ...
            layers.append( MultibranchTemporalBlock( in_channels, out_channels, self.ksizes,
                stride=1, dilation=dilation_size, padding = padding, dropout=dropout, relu_type = relu_type, dwpw=dwpw, phm=phm, n=n))
...
class TemporalBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, kernel_size, stride, dilation, padding, dropout=0.2,
                 symm_chomp = False, no_padding = False, relu_type = 'relu', dwpw=False, phm=False, n=4):
        ...
        if dwpw:
            if not phm:
                ...
                self.net = nn.Sequential(
                    ...
                    # -- second conv set within block
                    # -- dw
                    PHMConv1d(n, n_outputs, n_outputs, kernel_size, stride=stride,
                            padding=padding, dilation=dilation, groups=n_outputs, bias=False),
                    nn.BatchNorm1d(n_outputs),
                    Chomp1d(padding, True),
                    nn.PReLU(num_parameters=n_outputs) if relu_type == 'prelu' else Swish() if relu_type == 'swish' else nn.ReLU(inplace=True),
                    # -- pw
                    PHMConv1d(n, n_outputs, n_outputs, 1, 1, 0, bias=False),
                    nn.BatchNorm1d(n_outputs),
                    nn.PReLU(num_parameters=n_outputs) if relu_type == 'prelu' else Swish() if relu_type == 'swish' else nn.ReLU(inplace=True),
                    nn.Dropout(dropout),
                )
            ...
            if not phm:
                ...
                self.conv1 = PHMConv1d(n, n_inputs, n_outputs, kernel_size,
                                    stride=stride, padding=padding, dilation=dilation)
            ...
            if not phm:
                ...
                self.conv2 = PHMConv1d(n, n_outputs, n_outputs, kernel_size,
                                                stride=stride, padding=padding, dilation=dilation)
            ...
            if self.no_padding:
                ...
                self.net = nn.Sequential(self.conv1, self.batchnorm1, self.chomp1, self.relu1, self.dropout1, self.conv2, self.batchnorm2, self.chomp2, self.relu2, self.dropout2)

        if not phm:
            self.downsample = nn.Conv1d(n_inputs, n_outputs, 1) if n_inputs != n_outputs else None
        else:
            self.downsample = PHMConv1d(n, n_inputs, n_outputs, 1) if n_inputs != n_outputs else None
        if self.no_padding:
    ...
    def __init__(self, num_inputs, num_channels, tcn_options, dropout=0.2, relu_type='relu', dwpw=False, phm=False, n=4):
        ...
        for i in range(num_levels):
            ...
            layers.append( TemporalBlock(in_channels, out_channels, self.ksize, stride=1, dilation=dilation_size,
                                     ...
                                     no_padding = False, relu_type=relu_type, dwpw=dwpw), phm=phm, n=n)
